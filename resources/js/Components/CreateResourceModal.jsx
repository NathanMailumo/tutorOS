import React, { useState } from 'react';
import { useForm } from '@inertiajs/react';
import PrivateResource from './PrivateResource';
import PublicResource from './PublicResource';

export default function CreateResourceModal({ isOpen, onClose }) {
    const [activeTab, setActiveTab] = useState('private'); // 'private' | 'public'

    const { data, setData, post, processing, errors, reset } = useForm({
        course_name: '',
        resource_type: 'private',
        invite_emails: '',
    });

    if (!isOpen) return null;

    const handleTabSwitch = (type) => {
        setActiveTab(type);
        setData('resource_type', type);
    };

    const handleClose = () => {
        reset();
        onClose();
    };

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('resources.store'), {
            onSuccess: () => {
                reset();
                onClose();
            },
        });
    };

    return (
        <div className="animate-fadeIn fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm sm:p-4">
            {/* MODAL CARD */}
            <div className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border-2 border-black bg-white p-5 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] sm:rounded-3xl sm:p-6 sm:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                {/* HEADER & CLOSE BUTTON */}
                <div className="flex items-center justify-between border-b-2 border-black pb-3">
                    <h2 className="flex items-center gap-2 text-base font-black text-[#121212] sm:text-lg">
                        <span>Create Resource Space</span>
                    </h2>
                    <button
                        onClick={handleClose}
                        className="flex h-7 w-7 items-center justify-center rounded-lg border-2 border-black bg-red-500 text-xs font-black text-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-red-600"
                    >
                        ✕
                    </button>
                </div>

                {/* TYPE SELECTOR TABS */}
                <div className="mt-4 flex gap-1.5 rounded-2xl border-2 border-black bg-gray-100 p-1.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:gap-2">
                    <button
                        type="button"
                        onClick={() => handleTabSwitch('private')}
                        className={`flex-1 rounded-xl border-2 px-1 py-2 text-[11px] font-black transition-all sm:text-xs ${
                            activeTab === 'private'
                                ? 'border-black bg-white text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
                                : 'border-transparent text-gray-500 hover:text-black'
                        }`}
                    >
                        🔒 Private Space
                    </button>
                    <button
                        type="button"
                        onClick={() => handleTabSwitch('public')}
                        className={`flex-1 rounded-xl border-2 px-1 py-2 text-[11px] font-black transition-all sm:text-xs ${
                            activeTab === 'public'
                                ? 'border-black bg-[#2ED573] text-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
                                : 'border-transparent text-gray-500 hover:text-black'
                        }`}
                    >
                        🌐 Public / Shared
                    </button>
                </div>

                {/* TAB CONTENT */}
                {activeTab === 'private' ? (
                    <PrivateResource
                        data={data}
                        setData={setData}
                        onSubmit={handleSubmit}
                        onClose={handleClose}
                        processing={processing}
                        errors={errors}
                    />
                ) : (
                    <PublicResource
                        data={data}
                        setData={setData}
                        onSubmit={handleSubmit}
                        onClose={handleClose}
                        processing={processing}
                        errors={errors}
                    />
                )}
            </div>
        </div>
    );
}
